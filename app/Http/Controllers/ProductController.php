...
//
    ...
    public function designPDF($encoded_product_batch_id){
        $product_batch_id= base64_decode($encoded_product_batch_id);
        ...
        foreach ($products as $product) {
            ...
            $pdf->MultiCell(40, $product_name_cell_heigth, $product['product_name'], 1,'L');
    ...
    public function createPDF($encoded_product_batch_id)
    {
        $product_batch_id=base64_decode($encoded_product_batch_id);
        $products = Product::where('product_batch_id',$product_batch_id)
                            ->select('*')
        ...
        return view('products.preview_pdf',['pdfContent'=> $pdfContent,
        'encoded_product_batch_id'=>$encoded_product_batch_id]);
       
        
    }

    public function sendMailWithAttachment(Request $request,$product_batch_id)
    {   $product_batch_id= base64_decode($product_batch_id);
        ...
        $mailData = [
            ...
            'files' => [
                ...
                [
                    'content' => $pdfContent,
                    'name' => Carbon::now()->format('YmdHi') . 'products.pdf'
                ]
            ]
        ];

        Mail::to('owino.odero20@students.dkut.ac.ke')->
        send(new TestMail($mailData));
